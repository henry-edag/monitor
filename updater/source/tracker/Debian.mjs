...
import { isDatabase, Database, containsSoftware                            } from '../../source/Database.mjs';
...
const mergeAdvisory = function(vulnerability, advisory) {
...
const mergePackage = function(vulnerability, pkg_name, data) {
...
const Debian = function(database, updater) {
...
Debian.prototype = Object.assign({}, Emitter.prototype, {
	...
	'export': function() {
		...
		Object.values(this.__state['dsa']).forEach((dsa) => {
			...
			if (
				...
				dsa['cves'].forEach((cve_id) => {
					...
					let vulnerability = this.database.read(cve_id);
					if (vulnerability['is_edited'] === false) {

						mergeAdvisory.call(this, vulnerability, dsa);
				...
				let vulnerability = this.database.read(dsa['name']);
				if (vulnerability['is_edited'] === false) {
			...
			if (
				...
				dtsa['cves'].forEach((cve_id) => {
					...
					if (vulnerability['is_edited'] === false) {
						...
						mergeAdvisory.call(this, vulnerability, dtsa);
				...
				let vulnerability = this.database.read(dtsa['name']);
				if (vulnerability['is_edited'] === false) {
			...
			if (isObject(pkg_details) === true) {
				...
				Object.keys(pkg_details).forEach((cve_id) => {
					...
					if (vulnerability['is_edited'] === false) {
						...
						mergePackage.call(this, vulnerability, pkg_id, cve_details);
		...
		setTimeout(() => {
			...
			console.info('Debian: Export complete.');
			...
			this.webscraper.request('https://salsa.debian.org/security-tracker-team/security-tracker/-/raw/master/data/DTSA/list', (_, dtsa_buffer) => {
				...
				this.webscraper.request('https://security-tracker.debian.org/tracker/data/json', (data) => {
					...
					if (isObject(data) === true) {
						...
						console.info('Debian: Update complete.');
						this.emit('update');
