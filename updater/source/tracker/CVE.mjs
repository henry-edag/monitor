...
const mergeCVE = function(vulnerability, data) {
...
const CVE = function(database, updater) {
...
CVE.prototype = Object.assign({}, Emitter.prototype, {
	...
	'export': function() {
			...
			if (/^\/([0-9]{4})$/.test(year) === true) {
				...
				this.filesystem.index(year).sort().forEach((prefix) => {
					...
					this.filesystem.index(prefix, 'CVE-*.json').sort().map((path) => {
						...
						let vulnerability = this.database.read(entry['id']);
						if (
							vulnerability['is_edited'] === false
							&& STATE.includes(vulnerability['state']) === true
						) {

							mergeCVE.call(this, vulnerability, entry['data']);
		...
		setTimeout(() => {
			...
			console.info('CVE: Export complete.');
			this.emit('export');
